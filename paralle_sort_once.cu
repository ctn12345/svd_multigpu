...
void test17(){
    ...
    int* dev_pa,* dev_pb,* dev_pab;
    int* dev_pa1,* dev_pb1,* dev_pab1;
    int* dev_pa_1,* dev_pb_1,* dev_pab_1;
    int* dev_pa1_1,* dev_pb1_1,* dev_pab1_1;
...
#pragma region
    ...
    cudaSetDevice(gpu1);
    getRankNewNew_2<<<1,1024,0,stream2>>>(2*p1,dev_pab_1,dev_pa_1,dev_pb_1);
    getRankNewNew_1<<<1,1024,0,stream2>>>(p1,dev_pa1_1,dev_pb1_1,dev_pab1_1);
    ...
    int* host_index=(int*)malloc(sizeof(int)*2*(p+p1));
    fill_hostorder_total(raw_host_order,host_order_1,host_order_2,host_rawnorm_1,host_rawnorm_2,p,p1);
    ...
    //         printf("%f ",host_rawnorm_1[raw_host_order[u]]);
    //     }
    //     else{
    //         printf("%f ",host_rawnorm_2[raw_host_order[u]-2*p]);
    //     }
    // }
    // printf("\n");
    #pragma omp parallel
    ...
    while(!continue_flag){ 
        ...
        for(int i = 0;i < 2*p-1;++i){
            #pragma omp parallel
            {
                ...
                if(gpuid == 0){
                    cudaSetDevice(gpu0);
                    generate_jointG00_1<<<dimGrid77, 256,0,stream1>>>(dev_pab,dev_A, height, width_perdevice, p, q, dev_pairsOfEVD, dev_AiAi, dev_AiAj, dev_AjAj,i,  k, slice, sliceNum);    //&1.3
                    ...
                    cudaSetDevice(gpu1);
                    generate_jointG00_1<<<dimGrid77, 256,0,stream2>>>(dev_pab_1,dev_A_1, height, width_perdevice, p1, q, dev_pairsOfEVD_1, dev_AiAi_1, dev_AiAj_1, dev_AjAj_1,i,  k, slice, sliceNum);    //&1.3
        ...
        cudaSetDevice(gpu1);
        cudaMemcpyAsync(host_swap_data_1,dev_A_1+p1*k*height,sizeof(double)*p1*k*height,cudaMemcpyDeviceToHost,stream2);
        
        // cudaMemcpy(test_A,dev_A_1,sizeof(double)*width_perdevice*height,cudaMemcpyDeviceToHost);
        cudaSetDevice(gpu0);
        cudaMemcpyAsync(dev_A+p*k*height,host_swap_data_1,sizeof(double)*p*k*height,cudaMemcpyHostToDevice,stream1);
        // cudaStreamSynchronize(stream1);
        cudaSetDevice(gpu1);
        cudaMemcpyAsync(dev_A_1+p1*k*height,host_swap_data,sizeof(double)*p1*k*height,cudaMemcpyHostToDevice,stream2);
        // part2
        for(int i = 0;i < p;++i){
            #pragma omp parallel
            {
