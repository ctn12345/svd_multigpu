...
#pragma region
    ...
    cudaMemcpy(host_ab,p_ab,sizeof(int)*2*(p+p1)*(2*(p+p1)-1),cudaMemcpyDeviceToHost);
    ...
    while(!continue_flag){
        ...
        cudaMemcpy(test_Fnorm,dev_Fnorm,sizeof(double)*batch,cudaMemcpyDeviceToHost);
        // printf("\nstream 1 test_Fnorm:   %lf\n",test_Fnrom[0]);


        if(height >= 32){
            computeFnorm1<<<2 * p * batch, 128,0,stream2>>>(dev_A_1, dev_tempFnorm_1, p1, height/32, height, width_perdevice, k);
        }
        else{
            computeFnorm1<<<2 * p * batch, 128,0,stream2>>>(dev_A_1, dev_tempFnorm_1, p1, 1, height, width_perdevice, k);   
        }
        cudaDeviceSynchronize();
        computeFnorm2<<<batch, 32,0,stream2>>>(dev_tempFnorm_1, dev_Fnorm_1, p);  //&1.3
        double* test_Fnorm_1 = (double*)malloc(sizeof(double)*batch);
        cudaMemcpy(test_Fnorm_1,dev_Fnorm_1,sizeof(double)*batch,cudaMemcpyDeviceToHost);
